...
import { FaLinkedinIn, FaTwitter, FaGithub, FaInstagram, FaArrowRight  } from "react-icons/fa";
import { IoMdSend } from "react-icons/io";
import { FaHeart,FaPhone, FaMapPin } from "react-icons/fa";
      ...
      <div className="max-w-7xl mx-auto px-4">
        <div className="grid md:grid-cols-3 gap-8 mb-8">
              ...
              {navLinks.map(link => (
                <li key={link}>
                  <a href={`#${link.toLowerCase()}`} className="text-gray-400 hover:text-purple-500 transition-colors flex items-center gap-2">
                    <FaArrowRight  className="w-4 h-4" /> {link}
                  </a>
                </li>
              ))}
            </ul>
          ...
          <div>
            ...
            <div className="space-y-2 text-gray-400">
              <p className="flex items-center gap-2"><FaPhone className="w-4 h-4" /> +91 84770 27827</p>
              <p className="flex items-center gap-2"><MdMail className="w-4 h-4" /> shrwnkumar08@gmail.com</p>
              <p className="flex items-center gap-2"><FaMapPin className="w-4 h-4" /> Rudrapur, Uttarakhand</p>
            </div>
            <div className="flex gap-3 mt-4">
              {[FaLinkedinIn, FaTwitter, FaGithub, FaInstagram, IoMdSend].map((Icon, idx) => (
        ...
        <div className="border-t border-gray-800 pt-6 text-center text-gray-400">
          <p className="flex items-center justify-center gap-2">
            Made with <FaHeart className="w-4 h-4 text-red-500 animate-pulse" /> by Shrawan Kumar Singh
          </p>
        </div>
      </div>
    </footer>
